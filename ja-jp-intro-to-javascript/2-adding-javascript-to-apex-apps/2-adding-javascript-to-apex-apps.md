...
3. アプリを実行し、**製品**ページに移動します。 **バッグ**などの製品の**名前**をクリックします。次のようなフォームが表示されます。 
    ...
    現時点では、**製品イメージ**アイテムとイメージを表示する領域の両方が、**製品の可用性**アイテムが**No**に設定されているときに表示されます。次のステップでは、**製品の可用性**アイテムが**No**に設定されているときに、イメージアイテムと関連する領域を非表示にする動的アクションを実装します。

4. サンプルアプリのページ6、製品詳細ページのページデザイナーに移動します。左のレンダリングパネルで、**P6_PRODUCT_AVAIL**という名前のアイテムを右クリックし、**動的アクションの作成**を選択します。
